...
export const mapTypes = (types) => {
  ...
  if (typeMap.hasOwnProperty(types)) {
    return typeMap[types].map(type => {
      return {
        _ref: type.uuid,
        _type: 'reference',
        _key: nanoid(),
